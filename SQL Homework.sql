...
-- If the table needed to be recreated could pull in the code (see below) to recreate the missing table
-- copied from DDL for sakila.address 
...
SELECT f.title, 
COUNT(f.film_id) AS 'Rental Count'
FROM  film f
JOIN inventory i 
ON (f.film_id= i.film_id)
JOIN rental r 
